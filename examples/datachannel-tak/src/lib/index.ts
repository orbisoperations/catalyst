...
export async function runTask(env: any) {
	...
	const sendStuffToTak = async () => {
		...
		// airplane cot events
		const airplaneCOTEvents = data.airplanes
			? // eslint-disable-next-line @typescript-eslint/no-explicit-any
				data.airplanes.aircraftWithinDistance.map((item: any) => {
		...
		// earhtquakes
		const earthquakeCOTEvents = data.earthquakes
			? // eslint-disable-next-line @typescript-eslint/no-explicit-any
				data.earthquakes.earthquakes.map((item: any) => {
		...
		// line events
		const lineCOTEvents = data.line
			? // eslint-disable-next-line @typescript-eslint/no-explicit-any
				data.line.pings.map((item: any) => {
		...
		const takCOTEvents = takData
			? // eslint-disable-next-line @typescript-eslint/no-explicit-any
				takData.map((item: any) => {
