...
export class TAKDataManager extends DurableObject<Env> {
	...
	async alarm() {
		try {
			...
			const taskUUIDS = await runTask(this.env);
			...
			if (unitResp.status != 200) {
				...
				const allTAKPointsUnique = new Map<
					string,
					{
						uid: string;
						callsign: string;
				...
				const takGenPoints = Array.from(allTAKPointsUnique.entries())
					.filter(([uid, point]) => {
	...
	async getTAKPoints() {
		return (
			(await this.ctx.storage.get<{ uid: string; callsign: string; lat: number; lon: number; stale_time: string; type: string }[]>(
				'tak-uuids',
