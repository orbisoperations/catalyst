...
/**
 * Compliance Engine for channel compliance checking
 * Implements the core compliance logic for the MVP
 *
 * Uses a subset of Env bindings (AUTHX_TOKEN_API) for signing compliance tokens
 */
export class ComplianceEngine {
    constructor(private env: Pick<Env, 'AUTHX_TOKEN_API'>) {}

    /**
     * Checks compliance of a data channel endpoint with all configured tests
     * MVP implementation starts with authentication compliance, other tests to be added
     */
    async checkChannelCompliance(request: ComplianceRequest): Promise<ComplianceResult> {
        ...
        try {
            // 1. Authentication Compliance Test
            console.log(`[ComplianceEngine] Running authentication compliance test for channel ${request.channelId}`);
            const authTest = await this.testAuthenticationCompliance(request);
            tests.push(authTest);

            console.log(`[ComplianceEngine] Authentication test completed for channel ${request.channelId}`, {
                success: authTest.success,
                duration: `${authTest.duration}ms`,
                hasErrors: !!authTest.errorDetails,
            });

            // 2. Schema Introspection Test
            console.log(`[ComplianceEngine] Running schema introspection test for channel ${request.channelId}`);
            const introspectionTest = await this.testSchemaIntrospection(request);
            tests.push(introspectionTest);

            console.log(`[ComplianceEngine] Schema introspection test completed for channel ${request.channelId}`, {
                success: introspectionTest.success,
                duration: `${introspectionTest.duration}ms`,
                hasErrors: !!introspectionTest.errorDetails,
            });

            // 3. Federation Support Test
            console.log(`[ComplianceEngine] Running federation support test for channel ${request.channelId}`);
            const federationTest = await this.testFederationSupport(request);
            tests.push(federationTest);

            console.log(`[ComplianceEngine] Federation support test completed for channel ${request.channelId}`, {
                success: federationTest.success,
                duration: `${federationTest.duration}ms`,
                hasErrors: !!federationTest.errorDetails,
            });
            ...
            console.log(`[ComplianceEngine] Compliance check completed for channel ${request.channelId}`, {
                status,
            ...
            return {
                ...
                details: {
                    ...
                    tests: tests,
                    authenticationCompliance: authTest.success,
                    authenticationError: authTest.errorDetails,
                },
            };
        } catch (error) {
            const totalDuration = Date.now() - startTime;
    ...
    /**
     * Requests a system JWT token for compliance purposes
     * Shared helper for authentication compliance and introspection tests
     */
    ...
    /**
     * Tests authentication compliance for a data channel
     * Validates that the channel accepts valid tokens and rejects invalid ones
     */
    private async testAuthenticationCompliance(request: ComplianceRequest): Promise<TestResult> {
        const testStart = Date.now();

        console.log(`[ComplianceEngine:Auth] Starting authentication compliance test for channel ${request.channelId}`);

        try {
            // Step 1: Request a system JWT token for this channel
            const tokenResponse = await this.requestComplianceToken(request, 'channel-compliance');
            ...
            console.log(`[ComplianceEngine:Auth] Testing endpoint with VALID token`, {
                channelId: request.channelId,
                endpoint: request.endpoint,
            });
            ...
            console.log(`[ComplianceEngine:Auth] Valid token test result`, {
                channelId: request.channelId,
            ...
            console.log(`[ComplianceEngine:Auth] Testing endpoint with INVALID token`, {
                channelId: request.channelId,
                endpoint: request.endpoint,
            });
            ...
            console.log(`[ComplianceEngine:Auth] Invalid token test result`, {
                channelId: request.channelId,
            ...
            console.log(`[ComplianceEngine:Auth] Testing endpoint with NO token`, {
                channelId: request.channelId,
                endpoint: request.endpoint,
            });
            ...
            console.log(`[ComplianceEngine:Auth] No token test result`, {
                channelId: request.channelId,
            ...
            // All tests must pass for authentication compliance to succeed:
            ...
            const errorDetails = success
                ? undefined
                : this.formatAuthTestError(validTokenTest, invalidTokenTest, noTokenTest);

            console.log(`[ComplianceEngine:Auth] Authentication compliance summary`, {
                channelId: request.channelId,
                success,
            ...
            return {
                ...
                success: false,
                duration: Date.now() - testStart,
                errorDetails: `Authentication compliance test failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
            };
        }
    }

    /**
     * Tests GraphQL schema introspection capability
     * Validates that the channel returns a proper GraphQL schema via introspection query
     */
    private async testSchemaIntrospection(request: ComplianceRequest): Promise<TestResult> {
        const testStart = Date.now();

        console.log(
            `[ComplianceEngine:Introspection] Starting schema introspection test for channel ${request.channelId}`
        );

        try {
            // Step 1: Request a system JWT token for authenticated introspection
            const tokenResponse = await this.requestComplianceToken(request, 'channel-introspection');
          ...
          __schema {
            ...
            if (!jsonResponse.data.__schema) {
                const errorMsg = 'Response missing "__schema" field - introspection not supported or disabled';
                console.error(`[ComplianceEngine:Introspection] ${errorMsg}`, {
                    channelId: request.channelId,
                });
                return {
            ...
            // Success!
            console.log(`[ComplianceEngine:Introspection] Schema introspection test PASSED`, {
                channelId: request.channelId,
            ...
            return {
                ...
                success: false,
                duration: Date.now() - testStart,
                errorDetails: `Schema introspection test failed: ${errorMsg}`,
            };
        }
    }
     ...
     *
     * @param request - Compliance request containing channel details
     * @returns Test result with success/failure and timing information
     */
    private async testFederationSupport(request: ComplianceRequest): Promise<TestResult> {
        const testStart = Date.now();

        console.log(`[ComplianceEngine:Federation] Starting federation support test`, {
            channelId: request.channelId,
            endpoint: request.endpoint,
        });
        ...
        try {
            ...
            const tokenResult = await this.requestComplianceToken(request, 'Federation support test');
            ...
            console.log(`[ComplianceEngine:Federation] Sending SDL query to endpoint`, {
                channelId: request.channelId,
                endpoint: request.endpoint,
            });
            ...
            if (response.status !== 200) {
                ...
                console.error(`[ComplianceEngine:Federation] ${errorMsg}`, {
                    channelId: request.channelId,
                    status: response.status,
                });
                return {
            ...
            // Success!
            console.log(`[ComplianceEngine:Federation] Federation support test PASSED`, {
                channelId: request.channelId,
            ...
            const errorMsg = error instanceof Error ? error.message : 'Unknown error';
            console.error(`[ComplianceEngine:Federation] Test failed with error`, {
                channelId: request.channelId,
                error: errorMsg,
            });
            return {
                ...
                success: false,
                duration: Date.now() - testStart,
                errorDetails: `Federation support test failed: ${errorMsg}`,
            };
        }
    }
    ...
    private async testChannelWithToken(
        ...
        console.log(`[ComplianceEngine:Test] Starting ${testType} token test`, {
            endpoint,
            expectSuccess,
            hasToken: !!token,
        ...
        try {
          ...
          __schema {
            ...
            // For authentication compliance, we only care about authentication status
            ...
            if (expectSuccess) {
                ...
                const passed = !isAuthenticated;
                console.log(`[ComplianceEngine:Test] Invalid/no token test ${passed ? 'PASSED' : 'FAILED'}`, {
                    endpoint,
                    testType,
                    expected: 'rejected',
                    actual: isAuthenticated ? 'authenticated' : 'rejected',
    ...
    /**
     * Formats error message for authentication compliance test failures
     */
    private formatAuthTestError(
        ...
        if (!noTokenTest.accepted) {
            errors.push(`No token test failed: ${noTokenTest.error || 'Channel did not reject missing token'}`);
        }
