...
import { Catalyst, IssuedJWTRegistry } from '@catalyst/schema_zod';
...
describe("issued-jwt-registry unit tests", () => {
	...
	const newIJR: IssuedJWTRegistry =	await env.ISSUED_JWT_REGISTRY_DO.create(
			{
				id: "jwt_1",
				name: "my_first_jwt",
				description: "the first one we have ever tested",
				claims: ["claim1", "claim2"],
				expiry: Date.now() + (1000 * 60 * 60 * 24), //tomorrow
