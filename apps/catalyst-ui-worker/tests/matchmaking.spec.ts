...
describe("organization matchmaking", () => {
  describe("basic interactions", () => {
    it("create an invitation", async () => {
      ...
      const stub = env.ORG_MATCHMAKING.get(
        id,
      ) as DurableObjectStub<OrganizationMatchmakingDO>;
      ...
      if (invite.success) {
        ...
        if (sendInviteParse.success) {
          ...
          expect(sendInviteParse.data.status).toBe(
            OrgInviteStatus.enum.pending,
          );
        }
      }
    });
    ...
    it("respond to invitation", async () => {
      ...
      const failedResp = await stub.respond(
        "org1",
        invite.id,
        OrgInviteStatus.enum.accepted,
      );
      expect(failedResp.success).toBeFalsy;
      const acceptedResp = await stub.respond(
        "org2",
        invite.id,
        OrgInviteStatus.enum.accepted,
      );
    ...
    it("decline an invitation - org 1", async () => {
      ...
      const invite = OrgInvite.parse(inviteResp.invite);
      const org1DeclineResp = await stub.respond(
        "org1",
        invite.id,
        OrgInviteStatus.enum.declined,
      );
    ...
    it("decline an invitation - org 2", async () => {
      ...
      const invite = OrgInvite.parse(inviteResp.invite);
      const org2DeclineResp = await stub.respond(
        "org2",
        invite.id,
        OrgInviteStatus.enum.declined,
      );
