...
export const OrbisCard = (props: OrbisCardProps) => {
    const { children, size = 'md', paddingSize = 'md', header, headerActions, actions, ...cardProps } = props;
    return (
        <Card
            size={size}
            padding={paddingSize == 'sm' ? '8px' : paddingSize == 'md' ? '16px' : paddingSize == 'lg' ? '24px' : '0px'}
            variant={'outline'}
            shadow={'sm'}
            {...cardProps}
        >
...
export const ErrorCard = (props: { title: string; message: string; goBack?: () => void; retry?: () => void }) => {
    return (
        <OrbisCard
            title={props.title}
            header={props.title}
            actions={
                <Flex gap={5} align={'center'} justify={'space-between'}>
                    ...
                    {props.retry && <OrbisButton onClick={props.retry}>Retry</OrbisButton>}
                </Flex>
            }
        >
