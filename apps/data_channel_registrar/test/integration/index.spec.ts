...
describe('Data Channel Registrar as Durable Object integration tests', () => {
    ...
    it('accessing non existent data channel: exists in authzed but not in DO', async () => {
        ...
        // Also add user as regular user to ensure they have member permissions
        const addUserToOrg = await env.AUTHZED.addUserToOrg(TEST_ORG_ID, user.email);
        expect(addUserToOrg).toBeDefined();
        // Add user as admin to ensure they have read permissions
        ...
        const addDataChannelToOrg = await env.AUTHZED.addDataChannelToOrg(TEST_ORG_ID, 'dummy-data-channel-id');
        expect(addDataChannelToOrg).toBeDefined();
        // add org to data channel
        const addOrgToDataChannel = await env.AUTHZED.addOrgToDataChannel('dummy-data-channel-id', TEST_ORG_ID);
